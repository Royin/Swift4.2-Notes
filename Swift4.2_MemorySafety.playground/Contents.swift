...
// https://www.cnswift.org/memory-safety
...
/*
 ...
 在同一时间多处访问同一块内存会产生不可预料或者说不一致的行为。在 Swift 中，有好几种方式来修改跨越多行代码的值，从而可以在访问值的中间进行它自身的修改。
...
/*
 函数具有对其所有输入输出参数的长期写访问权。对输入输出形式参数的写入访问会在所有非输入输出形式参数计算之后开始，并持续到整个函数调用结束。如果有多个输入输出形式参数，那么写入访问会以形式参数出现的顺序开始。
 
 这种长期写入访问的一个后果是您无法访问作为输入输出传递的原始变量，即使范围规则和访问控制允许它 - 任何对原始数据的访问都会产生冲突。比如说：
 */
var stepSize = 1

func increment(_ number: inout Int) {
    number += stepSize
    // 函数调用结束前，输入输出参数number的写入访问一直持续
}

increment(&stepSize)

/*
 在上面的代码中，stepSize是一个全局变量，它通常可以从内部访问increment(_:)。但是，读访问stepSize与写访问number重叠。如在下文中的图所示，两者number和stepSize指代相同的位置在存储器中。读和写访问是相同的内存，它们重叠，产生冲突。
 */

// 一种解决这个冲突的办法是显式地做一个 stepSize 的拷贝：
var copyOfStepSize = stepSize // 值类型，复制了一份,读访问

stepSize = 454
print("\(copyOfStepSize) \(stepSize)")

increment(&copyOfStepSize)
stepSize = copyOfStepSize

/*
 当你stepSize在调用之前复制一个副本时increment(_:)，很明显，copyOfStepSize当前步长会增加值。读访问在写访问开始之前结束，因此不存在冲突。
...
//balance(&playerOneScore, &playerOneScore) 报错
...
/*
 在上面的例子中，调用 Oscar 玩家的 shareHealth(with:) 方法分享血条给 Maria 玩家不会造成冲突。
 在方法调用的过程中只有一个写入访问到 oscar 因为 oscar 是异变方法中 self 值，并且在同一时间内只有一个写入访问到 maria 因为 maria 是以输入输出形式参数传递的。如下面的图例所示，他们访问了内存中不同的地址。就算他们的写入访问是同时发生的，但不会冲突。
 */

// 如果你把 oscar 作为实际参数传递给 shareHealth(with:) ，就出现了冲突：
//oscar.shareHealth(with: &oscar) // 报错
...
/*
 像是结构体、元组以及枚举浙西额类型都是由独立值构成，比如结构体的属性或者元组的元素。由于这些都是值类型，改变任何一个值都会改变整个类型，意味着读或者写访问到这些属性就需要对整个值进行读写访问。比如说，对元组的元素进行重叠写入访问就会产生冲突：
 */
...
/*
 实际上，大多数对接口固体属性的访问可安全的重叠。比如，如果上边变量 holly 变成局部变量而不是全局变量，那么编译器就可以保证重叠访问结构体的存储属性是安全的：
 */
func someFunction() {
    var oscar = Player(name: "Oscar", health: 10, energy: 10)
